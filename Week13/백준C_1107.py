...
#만약 고장난 버튼이 0개 이상이라면
if b_Button > 0:
    b_Button_list = list(map(int, input().split()))
    # 고장난 리모컨 버튼 제거
    for i in b_Button_list:
        remoteNum.remove(i)
...
for channel in range(1000000):
    for j in range(len(str(channel))): #각 채널을 '문자열'로 변환해서 그 길이만큼 돈다.
        if int(str(channel)[j]) not in remoteNum: #만약 채널의 각각의 자리수가 리모컨의 고장나 있는 버튼이라면
            ...
            result = min(result, len(str(channel)) + abs(channel - n)) #우선 해당 채널에서 입력받은 채널까지 버튼을 눌리는 총 횟수와 기존의 100번부터 입력받은 채널을 +- 로 이동하는 횟수와 비교해서 최솟값을 대입 -> 그 후 채널마다 계속해서 더 작은 값을 결과값에 대입
